...
public class Calculator {
	...
	final private static String[] logicalOperators = new String[] {AND, OR, EQUAL, NOT_EQUAL, LOWER, LOWER_EQUAL, GREATER, GREATER_EQUAL};
	...
	public static final boolean booleanCalculate(boolean leftValue, String operator, boolean rightValue) {
		boolean result = true;
	...
	public static final Object calculate(double leftValue, String operator, double rightValue) {
		...
		if (isArithmetic(operator)) {
			...
			AbstractStackHelper.stopExecution("Operator '" + operator + "' could not be recognized or operand(s) is/are not integer.");
		}
		return result;
	}
	...
	public static final Object calculate(boolean leftValue, String operator, boolean rightValue) {
		...
		if (isLogical(operator)) {
			result = booleanCalculate(leftValue, operator, rightValue);
		} else {
		...
		if (isArithmetic(operator)) {
			switch (type) {
				case "integer":
					...
					break;
				case "double":
					result = arithmeticCalculate((double) leftValue, operator, (double) rightValue);
					break;
				default:
			...
			if (type.equals("integer")) {
				...
				AbstractStackHelper.stopExecution("Type '" + type + "' is not applicable to calculate binary. Must be integer.");
			}
		} else if (isLogical(operator)) {
			if (type == "boolean") {
				result = booleanCalculate((boolean) leftValue, operator, (boolean) rightValue);
			} else {
				AbstractStackHelper.stopExecution("Type'" + type + "' is not applicable to calculate boolean. Must be boolean.");
			}
		}
		return result;
	}
